...
class OnebusawaySdkClientImpl(
    ...
    private val async: OnebusawaySdkClientAsync by lazy { OnebusawaySdkClientAsyncImpl(clientOptions) }

    private val withRawResponse: OnebusawaySdkClient.WithRawResponse by lazy { WithRawResponseImpl(clientOptions) }

    private val agenciesWithCoverage: AgenciesWithCoverageService by lazy { AgenciesWithCoverageServiceImpl(clientOptionsWithUserAgent) }

    private val agency: AgencyService by lazy { AgencyServiceImpl(clientOptionsWithUserAgent) }

    private val vehiclesForAgency: VehiclesForAgencyService by lazy { VehiclesForAgencyServiceImpl(clientOptionsWithUserAgent) }

    private val config: ConfigService by lazy { ConfigServiceImpl(clientOptionsWithUserAgent) }

    private val currentTime: CurrentTimeService by lazy { CurrentTimeServiceImpl(clientOptionsWithUserAgent) }

    private val stopsForLocation: StopsForLocationService by lazy { StopsForLocationServiceImpl(clientOptionsWithUserAgent) }

    private val stopsForRoute: StopsForRouteService by lazy { StopsForRouteServiceImpl(clientOptionsWithUserAgent) }

    private val stopsForAgency: StopsForAgencyService by lazy { StopsForAgencyServiceImpl(clientOptionsWithUserAgent) }

    private val stop: StopService by lazy { StopServiceImpl(clientOptionsWithUserAgent) }

    private val stopIdsForAgency: StopIdsForAgencyService by lazy { StopIdsForAgencyServiceImpl(clientOptionsWithUserAgent) }

    private val scheduleForStop: ScheduleForStopService by lazy { ScheduleForStopServiceImpl(clientOptionsWithUserAgent) }

    private val route: RouteService by lazy { RouteServiceImpl(clientOptionsWithUserAgent) }

    private val routeIdsForAgency: RouteIdsForAgencyService by lazy { RouteIdsForAgencyServiceImpl(clientOptionsWithUserAgent) }

    private val routesForLocation: RoutesForLocationService by lazy { RoutesForLocationServiceImpl(clientOptionsWithUserAgent) }

    private val routesForAgency: RoutesForAgencyService by lazy { RoutesForAgencyServiceImpl(clientOptionsWithUserAgent) }

    private val scheduleForRoute: ScheduleForRouteService by lazy { ScheduleForRouteServiceImpl(clientOptionsWithUserAgent) }

    private val arrivalsAndDeparturesForLocation: ArrivalsAndDeparturesForLocationService by lazy { ArrivalsAndDeparturesForLocationServiceImpl(clientOptionsWithUserAgent) }

    private val arrivalAndDeparture: ArrivalAndDepartureService by lazy { ArrivalAndDepartureServiceImpl(clientOptionsWithUserAgent) }

    private val trip: TripService by lazy { TripServiceImpl(clientOptionsWithUserAgent) }

    private val tripsForLocation: TripsForLocationService by lazy { TripsForLocationServiceImpl(clientOptionsWithUserAgent) }

    private val tripDetails: TripDetailService by lazy { TripDetailServiceImpl(clientOptionsWithUserAgent) }

    private val tripForVehicle: TripForVehicleService by lazy { TripForVehicleServiceImpl(clientOptionsWithUserAgent) }

    private val tripsForRoute: TripsForRouteService by lazy { TripsForRouteServiceImpl(clientOptionsWithUserAgent) }

    private val reportProblemWithStop: ReportProblemWithStopService by lazy { ReportProblemWithStopServiceImpl(clientOptionsWithUserAgent) }

    private val reportProblemWithTrip: ReportProblemWithTripService by lazy { ReportProblemWithTripServiceImpl(clientOptionsWithUserAgent) }

    private val searchForStop: SearchForStopService by lazy { SearchForStopServiceImpl(clientOptionsWithUserAgent) }

    private val searchForRoute: SearchForRouteService by lazy { SearchForRouteServiceImpl(clientOptionsWithUserAgent) }
    ...
    override fun withOptions(modifier: (ClientOptions.Builder) -> Unit): OnebusawaySdkClient = OnebusawaySdkClientImpl(clientOptions.toBuilder().apply(modifier).build())
    ...
    override fun arrivalsAndDeparturesForLocation(): ArrivalsAndDeparturesForLocationService = arrivalsAndDeparturesForLocation
    ...
    class WithRawResponseImpl internal constructor(
        ...
        private val agenciesWithCoverage: AgenciesWithCoverageService.WithRawResponse by lazy { AgenciesWithCoverageServiceImpl.WithRawResponseImpl(clientOptions) }

        private val agency: AgencyService.WithRawResponse by lazy { AgencyServiceImpl.WithRawResponseImpl(clientOptions) }

        private val vehiclesForAgency: VehiclesForAgencyService.WithRawResponse by lazy { VehiclesForAgencyServiceImpl.WithRawResponseImpl(clientOptions) }

        private val config: ConfigService.WithRawResponse by lazy { ConfigServiceImpl.WithRawResponseImpl(clientOptions) }

        private val currentTime: CurrentTimeService.WithRawResponse by lazy { CurrentTimeServiceImpl.WithRawResponseImpl(clientOptions) }

        private val stopsForLocation: StopsForLocationService.WithRawResponse by lazy { StopsForLocationServiceImpl.WithRawResponseImpl(clientOptions) }

        private val stopsForRoute: StopsForRouteService.WithRawResponse by lazy { StopsForRouteServiceImpl.WithRawResponseImpl(clientOptions) }

        private val stopsForAgency: StopsForAgencyService.WithRawResponse by lazy { StopsForAgencyServiceImpl.WithRawResponseImpl(clientOptions) }

        private val stop: StopService.WithRawResponse by lazy { StopServiceImpl.WithRawResponseImpl(clientOptions) }

        private val stopIdsForAgency: StopIdsForAgencyService.WithRawResponse by lazy { StopIdsForAgencyServiceImpl.WithRawResponseImpl(clientOptions) }

        private val scheduleForStop: ScheduleForStopService.WithRawResponse by lazy { ScheduleForStopServiceImpl.WithRawResponseImpl(clientOptions) }

        private val route: RouteService.WithRawResponse by lazy { RouteServiceImpl.WithRawResponseImpl(clientOptions) }

        private val routeIdsForAgency: RouteIdsForAgencyService.WithRawResponse by lazy { RouteIdsForAgencyServiceImpl.WithRawResponseImpl(clientOptions) }

        private val routesForLocation: RoutesForLocationService.WithRawResponse by lazy { RoutesForLocationServiceImpl.WithRawResponseImpl(clientOptions) }

        private val routesForAgency: RoutesForAgencyService.WithRawResponse by lazy { RoutesForAgencyServiceImpl.WithRawResponseImpl(clientOptions) }

        private val scheduleForRoute: ScheduleForRouteService.WithRawResponse by lazy { ScheduleForRouteServiceImpl.WithRawResponseImpl(clientOptions) }

        private val arrivalsAndDeparturesForLocation: ArrivalsAndDeparturesForLocationService.WithRawResponse by lazy { ArrivalsAndDeparturesForLocationServiceImpl.WithRawResponseImpl(clientOptions) }

        private val arrivalAndDeparture: ArrivalAndDepartureService.WithRawResponse by lazy { ArrivalAndDepartureServiceImpl.WithRawResponseImpl(clientOptions) }

        private val trip: TripService.WithRawResponse by lazy { TripServiceImpl.WithRawResponseImpl(clientOptions) }

        private val tripsForLocation: TripsForLocationService.WithRawResponse by lazy { TripsForLocationServiceImpl.WithRawResponseImpl(clientOptions) }

        private val tripDetails: TripDetailService.WithRawResponse by lazy { TripDetailServiceImpl.WithRawResponseImpl(clientOptions) }

        private val tripForVehicle: TripForVehicleService.WithRawResponse by lazy { TripForVehicleServiceImpl.WithRawResponseImpl(clientOptions) }

        private val tripsForRoute: TripsForRouteService.WithRawResponse by lazy { TripsForRouteServiceImpl.WithRawResponseImpl(clientOptions) }

        private val reportProblemWithStop: ReportProblemWithStopService.WithRawResponse by lazy { ReportProblemWithStopServiceImpl.WithRawResponseImpl(clientOptions) }

        private val reportProblemWithTrip: ReportProblemWithTripService.WithRawResponse by lazy { ReportProblemWithTripServiceImpl.WithRawResponseImpl(clientOptions) }

        private val searchForStop: SearchForStopService.WithRawResponse by lazy { SearchForStopServiceImpl.WithRawResponseImpl(clientOptions) }

        private val searchForRoute: SearchForRouteService.WithRawResponse by lazy { SearchForRouteServiceImpl.WithRawResponseImpl(clientOptions) }

        private val block: BlockService.WithRawResponse by lazy { BlockServiceImpl.WithRawResponseImpl(clientOptions) }

        private val shape: ShapeService.WithRawResponse by lazy { ShapeServiceImpl.WithRawResponseImpl(clientOptions) }

        override fun withOptions(modifier: (ClientOptions.Builder) -> Unit): OnebusawaySdkClient.WithRawResponse = OnebusawaySdkClientImpl.WithRawResponseImpl(clientOptions.toBuilder().apply(modifier).build())

        override fun agenciesWithCoverage(): AgenciesWithCoverageService.WithRawResponse = agenciesWithCoverage

        override fun agency(): AgencyService.WithRawResponse = agency

        override fun vehiclesForAgency(): VehiclesForAgencyService.WithRawResponse = vehiclesForAgency
        ...
        override fun routeIdsForAgency(): RouteIdsForAgencyService.WithRawResponse = routeIdsForAgency

        override fun routesForLocation(): RoutesForLocationService.WithRawResponse = routesForLocation
        ...
        override fun arrivalsAndDeparturesForLocation(): ArrivalsAndDeparturesForLocationService.WithRawResponse = arrivalsAndDeparturesForLocation
        ...
        override fun reportProblemWithStop(): ReportProblemWithStopService.WithRawResponse = reportProblemWithStop

        override fun reportProblemWithTrip(): ReportProblemWithTripService.WithRawResponse = reportProblemWithTrip
